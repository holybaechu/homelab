...
TOPOLOGY = ROOT / "infra" / "ansible" / "inventory" / "prod" / "topology.json"
...
def _create_only_lxc_changes(plan: dict[str, Any]) -> list[str]:
    ...
    for resource in _resource_changes(plan):
        ...
        if not address.startswith("module.target_lxc[") or not address.endswith(
            LXC_RESOURCE_SUFFIX
        ):
            continue
...
def _expected_lxc_count() -> int:
    ...
    containers = topology["all"]["children"]["debian"]["hosts"]
    if not isinstance(containers, dict) or not containers:
        raise ValueError(f"no Debian LXC hosts found in {TOPOLOGY}")
...
def main(argv: list[str]) -> int:
    ...
    if destructive:
        ...
        if _truthy(os.environ.get("ALLOW_TOFU_DESTROY")):
            print(
                "ALLOW_TOFU_DESTROY is set; allowing destructive plan.",
                file=sys.stderr,
            )
            return 0
    ...
    if create_only_lxcs and _truthy(os.environ.get("ALLOW_EMPTY_STATE_BOOTSTRAP")):
        print(
            "ALLOW_EMPTY_STATE_BOOTSTRAP is set; allowing create-only LXC plan.",
            file=sys.stderr,
        )
