...
REQUIRED_ENV = {
    ...
    "copyparty_password_hash_salt": "COPYPARTY_PASSWORD_HASH_SALT",
}
...
def load_copyparty_users() -> list[dict[str, Any]]:
    ...
    for index, user in enumerate(users):
        ...
        if not user["password_hash"].startswith("+"):
            raise SystemExit(
                f"copyparty user {user['name']!r} password_hash must be generated by copyparty --ah-alg scrypt and start with '+'"
            )

    return users


def validate_copyparty_password_hash_salt(mapping: dict[str, Any]) -> None:
    salt = mapping["copyparty_password_hash_salt"]
    if len(salt) < 16:
        raise SystemExit("COPYPARTY_PASSWORD_HASH_SALT must be at least 16 characters")


def build_mapping() -> dict[str, Any]:
    mapping = {var_name: require_env(env_name) for var_name, env_name in REQUIRED_ENV.items()}
    validate_copyparty_password_hash_salt(mapping)
