...
def load_copyparty_users() -> list[dict[str, Any]]:
    ...
    for index, user in enumerate(users):
        ...
        if "password" in user:
            raise SystemExit(
                "COPYPARTY_USERS_JSON must use password_hash, not plaintext password"
            )
        if "password" not in user and (
            not isinstance(user.get("password_hash"), str) or not user["password_hash"]
        ):
            raise SystemExit(
                f"copyparty user {user.get('name', index + 1)!r} must include password_hash"
            )
