...
for service in ${qbittorrent_services}; do
  ...
  if test "${effective_mods_status}" -ne 0; then
    ...
    fail "${service} environment: effective DOCKER_MODS=<invalid or unavailable; value suppressed>; expected one official VueTorrent mod with a semantic-version tag"
  fi
done
