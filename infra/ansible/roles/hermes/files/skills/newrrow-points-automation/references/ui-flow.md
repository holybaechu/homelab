...
1. On every Newrrow run, use the active Hermes browser/agent-browser session. If no Newrrow tab is available, open `https://gbsm.newrrow.com/csr-platform/home` with `agent-browser open "$NEWRROW_HOME_URL"` or the Hermes browser navigation tool. Do not wait for a separate "open the website" request when no Newrrow tab is available.
