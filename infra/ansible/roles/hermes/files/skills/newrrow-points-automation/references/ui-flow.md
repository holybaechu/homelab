...
1. On every Newrrow run, use the active Hermes browser-tool session. If no Newrrow tab is available, call `browser_navigate` on `https://gbsm.newrrow.com/csr-platform/home`. Do not wait for a separate "open the website" request when no Newrrow tab is available. Public Newrrow URLs should stay on Hermes' configured cloud browser path (Browserbase in homelab); do not use bare `agent-browser open` for runtime operation.
...
| Login | Use `browser_navigate` for the public Newrrow URL. If login is visible, call `newrrow_browser_login`; it reads `NEWRROW_USERNAME_REF` and `NEWRROW_PASSWORD_REF` internally without returning secrets. |
...
3. If `browser_click` on a visible `시간 슬롯` ref does not open the modal, use `browser_snapshot`/`browser_vision` to identify the empty gap and then use `browser_console` to click the element at the gap center, for example `document.elementFromPoint(x, y)?.click()`. This keeps the fallback inside the Hermes browser-tool session instead of dropping to bare `agent-browser mouse` commands. A coordinate click was verified to open the timetable modal for an empty `01:10 PM` gap where direct ref-clicks were no-ops.
...
1. Stay on the Hermes browser-tool route. Call `browser_navigate` on `https://gbsm.newrrow.com/csr-platform/home` and inspect the snapshot.
2. If the login page is visible, call `newrrow_browser_login`. The plugin reads `NEWRROW_USERNAME_REF` and `NEWRROW_PASSWORD_REF` via `op read` inside the tool handler and injects them into the active Browserbase/CDP-backed browser session without returning secret values.
3. Do not call `browser_type` with raw passwords or `op read` output; those arguments/results are model-visible. Do not use bare `agent-browser open`, `agent-browser auth save`, or `agent-browser auth login` for Newrrow runtime operation because that bypasses Hermes browser routing and can silently use local Chromium for this public URL.
4. `scripts/newrrow-login.sh` is only a credential preflight helper for deployment/debugging. It verifies the 1Password refs and prints `newrrow_1password_refs_ready`; it does not log into Newrrow.
5. Do not inspect cookies, local storage, browser profile files, or hidden credential stores.
6. If 1Password cannot read the configured refs, if Newrrow requires 2FA, or if a CAPTCHA/security prompt appears, stop and ask the user to complete that visible step or update the refs.
