...
`openclaw-setup` checkout. Runtime state and the Gateway token remain outside
Git. The exact former Docker Gateway is retained stopped on VMID 110 behind a
permanent source hold for explicit tracked rollback only.
