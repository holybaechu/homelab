...
and the workload Compose projects on the application LXC, including the
isolated OpenClaw foundation, and the separate Arcane management control plane. It also
...
OpenClaw deployment material is public in `apps/compose/openclaw`; its private
Git-managed configuration is a read-only bind source from the separate
`openclaw-setup` repository. Runtime state and the Gateway token remain outside
both repositories.
