...
- Arcane deploys and operates `platform`, `media`, and `code`. Its former
  `openclaw` sync is retired and must remain absent after native cutover.
...
4. Confirm Arcane discovers exactly `platform`, `media`, and `code` from
   `/opt/homelab-compose`; the retained OpenClaw project must remain absent.
   `arcane-control` must not be a managed project.
...
`apps/compose/media`, or `apps/compose/code`, CD selects the `arcane` scope.
OpenClaw rollback-manifest or documentation changes deliberately select the
full marker-aware Ansible path and must not attempt to recreate the retired
Arcane project.
