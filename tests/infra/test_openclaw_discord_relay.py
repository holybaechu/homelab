...
def test_validation_keeps_ctf_transfer_data_out_of_the_relay_identity():
    ...
    for required in (
        ...
        "InaccessiblePaths={{ openclaw_ctf_plugin_root }}",
        "InaccessiblePaths={{ openclaw_ctf_docker_cli_path | dirname }}",
        "test ! -e '{{ openclaw_secret_root }}/ctf_openai_api_key'",
        "! systemctl cat openclaw-ctf-gateway.service | grep -Eq '(^|[^A-Z_])OPENAI_API_KEY='",
        "! systemctl cat openclaw-gateway.service | grep -Eq '(^|[^A-Z_])OPENAI_API_KEY='",
        "! systemctl cat openclaw-discord-relay.service | grep -Eq '(^|[^A-Z_])OPENAI_API_KEY='",
