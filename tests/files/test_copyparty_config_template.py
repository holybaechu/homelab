...
def render_copyparty_config(copyparty_users):
    return render_role_template(
        ...
        copyparty_password_hash_salt="example-stable-copyparty-salt",
...
def test_copyparty_config_renders_only_hashed_accounts():
    rendered = render_copyparty_config(
        [{"name": "holybaechu", "password_hash": "+example-hash"}]
    )

    assert "[accounts]" in rendered
    assert "holybaechu: +example-hash" in rendered
...
def test_copyparty_config_enables_scrypt_password_hashing_with_stable_salt():
    rendered = render_copyparty_config(
        [{"name": "holybaechu", "password_hash": "+example-hash"}]
    )

    assert "ah-alg: scrypt" in rendered
    assert "ah-salt: example-stable-copyparty-salt" in rendered


def test_copyparty_secret_users_get_shared_read_only_access():
    rendered = render_copyparty_config(
        [
            {"name": "holybaechu", "password_hash": "+owner"},
            {"name": "siregon72", "password_hash": "+guest1"},
            {"name": "ezmin1104", "password_hash": "+guest2"},
            {"name": "sieon", "password_hash": "+guest3"},
        ]
    )
    ...
    assert "siregon72: +guest1" in accounts_section
    assert "ezmin1104: +guest2" in accounts_section
    assert "sieon: +guest3" in accounts_section
