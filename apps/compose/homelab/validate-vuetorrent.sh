...
if docker compose exec -T "${service}" \
  test -f /vuetorrent/public/index.html >/dev/null 2>&1; then
...
effective_mods="$(
  docker compose exec -T "${service}" printenv DOCKER_MODS 2>/dev/null
)"
...
effective_mods_matches="$(
  printf '%s\n' "${effective_mods}" \
    | grep -Ec '^ghcr\.io/vuetorrent/vuetorrent-lsio-mod:[0-9]+\.[0-9]+\.[0-9]+@sha256:[0-9a-f]{64}$'
)"
...
if test "${effective_mods_status}" -ne 0; then
  ...
  pass "${service} environment: effective DOCKER_MODS=${effective_mods}"
else
  fail "${service} environment: effective DOCKER_MODS=<invalid or unavailable; value suppressed>; expected one official VueTorrent mod with an exact semantic-version tag and sha256 digest"
fi
...
init_logs="$(docker compose logs --no-color --tail 80 "${service}" 2>&1)"
init_logs_status=$?
if test "${init_logs_status}" -eq 0; then
  ...
  printf 'INFO %s init-log summary: VueTorrent mentioned=%s\n' \
    "${service}" "${vuetorrent_mentioned}"
  printf 'INFO %s init-log summary: Docker mod mentioned=%s\n' \
    "${service}" "${docker_mod_mentioned}"
  printf 'INFO %s init-log summary: error/failure mentioned=%s\n' \
    "${service}" "${error_failure_mentioned}"
else
  printf 'INFO %s init-log summary unavailable (docker compose logs command failed)\n' \
    "${service}"
fi
